...
<template>
  <div class="blog-main blog-container">
    ...
    <p>
      ...
      scales and chords. Simply clicking on any note produces its corresponding
      sound. Additionally, users can play notes and chords using their laptop
      keyboard. The app provides various sound effects and allows for
      customization of note durations, enhancing the overall experience.
    </p>
    <h2 ref="part1">Styling</h2>
    <p>
      To design the piano, I opted for a grid-based approach. The piano
      container is displayed as a grid, with the grid-template-columns property
      set to (1fr, 1.3fr, 1fr, 1.3fr, 1fr). This configuration spans the entire
      length of the keyboard. Each 1fr unit accommodates three white notes and
      two black notes within it. Similarly, each 1.3fr unit accommodates four
      white notes and three black notes within it. The black frame of the piano
      is created using a border, with the left and right borders set to 3rem
      solid black, and the top border set to a thick 5rem. Each of these grid
      items also utilizes a grid display. For the items with three white notes,
      the grid-template-columns property is set to repeat(3, 1fr). As for the
      highlighted item in the image below, the grid-template-columns property is
      set to repeat(4, 1fr), which evenly spaces out the white notes.
    </p>
    <BlogImage
    ...
    <p>
      ...
      each parent div element is set to relative. This approach ensures that the
      black notes are not part of the grid structure but instead placed inside
      the corresponding elements. To handle the black notes, five classes are
      required, each with a different left position. Here is an example of one
      of these classes:
    </p>
    <CodeSnippet
      class="mb-5"
    ...
    <p>
      To bring in a sound, create an object, and choose the sound. In the below
      example. We create a sampler instance which is a piano sound.
    </p>
    <CodeSnippet
      lang="js"
    ...
    <p>
      ...
      takes two arguments. First is the note you want to get played. The second
      is the duration of the note. To play middle C(middle note on a piano),
      This would get called. Using the instance example above.
    </p>
    ...
    <p>
      ...
      Firstly, we need to select all the white notes in the DOM. We can achieve
      this by using the querySelectorAll method, which returns a node list
      containing all the matching elements. Next, we can iterate through the
      node list using the forEach method to perform actions on each white note.
      Similarly, we can target the black notes using the same approach.
    </p>
    <CodeSnippet
      lang="js"
    ...
    <p>
      ...
      argument when the triggerAttckRelease gets called. The second argument is
      whatever gets selected by the user(duration selection in the top right of
      the screen).
    </p>
    <CodeSnippet
      lang="js"
...
sound.triggerAttackRelease (note, duration);
    ...
    <p>
      ...
      represent the ID of a specific note. When a key is pressed down, a keydown
      event will trigger the same triggerAttackRelease function, but with a
      different first argument, which corresponds to the note ID.
    </p>
    <CodeSnippet
      lang="js"
...
await Tone.start();" />
    <p>
      ...
      chord (e.g., "G") and the values contain a list of the notes in that chord
      (e.g., ["G", "B", "D"]). To retrieve the desired notes, we will iterate
      through these chords using a loop. The following code snippet is within
      the scope of the previous snippet.
    </p>
    <CodeSnippet
      lang="js"
      code="let notes = [];
...
document.getElementById (notes[2].id).style.background = 'rgb(255, 214, 138)';
    ...
    <p>
      In this app, you have the option to choose from four different sounds. The
      default and highest-quality sound is the piano, which provides the best
      auditory experience. However, I've also included three additional sounds
      to add variety. These sounds are imported from the Tone JS library.
    </p>
    <CodeSnippet
      lang="js"
...
sound = piano;
    ...
    <p>
      Another option available in this app is the ability to change the duration
      of notes. There are five different durations to choose from, with the
      default set to an 8th note. To handle this functionality, we have a global
      variable called 'duration' that changes whenever a different duration is
      selected. When the 'triggerAttackRelease()' function is called, this
      'duration' variable is used as the second argument, allowing us to control
    ...
    <p>
      ...
      open to add more features in the future. One potential idea is to program
      a song playback functionality. Additionally, there is room for improvement
      in terms of the mobile user experience, as the keys are currently too
      small to be easily played. Thank you for taking the time to read about my
      project!
    </p>
  </div>
</template>
